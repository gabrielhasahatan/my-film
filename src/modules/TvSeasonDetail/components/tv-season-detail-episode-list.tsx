...
const TvSeasonDetailEpisodeList = () => {
  ...
  const { data, isLoading, } = useSWR(`tv_${detail.id}_season_${seasonSelect}_episode`, fetcher, {
    onSuccess: () => {
        ...
        <>
          <FieldGroup className="w-full max-w-[150px] ">
            <Field className="bg-white/50 rounded-lg">
              <Select onValueChange={setSeasonSelect} defaultValue={detail.seasons[detail.seasons[0].season_number === 0 ? defaultSeason : defaultSeason - 1].season_number.toString()}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent
