...
import { indexMovieDiscover } from "../lib/action"
...
import MovieDiscoverEmpty from "./movie-discover-empty"
import { MovieDiscoverPagination } from "./movie-discover-pagination"
...
const MovieDiscoverList = () => {
  ...
  const fetcher = async () => {
    const result = await indexMovieDiscover({ page: currentPage.toString() })
    if (result.success) {
      return result.data
    } else {
  ...
  const { data, error, isLoading } = useSWR(`discover_movie_${currentPage.toString()}`, fetcher)
        ...
        <>
          {totalData > 0 ?
            <div className="mt-20 md:px-4">
              ...
              <MovieDiscoverPagination totalPages={data?.total_pages ?? 1} currentPage={currentPage} onPageChange={(newPage) => {
