...
import TvSeasonDropdownButton from "./tv-season-dropdown-button"
...
const TvDropDownMemo = memo(({ onChangeSelect }: { onChangeSelect: () => void }) => {
  return <TvSeasonDropdownButton onChangeSelect={onChangeSelect} />
})

const TvEpisodeDetailList = ({ id }: { id: string }) => {
  const [seasonSelect, setSeasonSelect] = useState("1")
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null)
  const [showDropdown, setShowDropdown] = useState(false)

  const fetcher = async () => {
    const result = await TvEpisodesList({ seasonId: seasonSelect, seriesId: id })
      ...
      <div className="bg-black px-4 pt-4 pb-8">
        {
          showDropdown &&
          <>
            <TvSeasonDropdownButton onChangeSelect={setSeasonSelect} />
