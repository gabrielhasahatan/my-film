...
import { RecommendationListTv } from "../lib/action"
import { useTvSeasonDetailContext } from "@/modules/TvSeasonDetail/components/tv-season-detail-provider"
import { PaginationTvRecommendation } from "./tv-recommendation-pagination"
import TvRecommendationsEmpty from "./tv-recommendations-empty"

const TvRecommendationsList = () => {
  const [currentPage, setCurrentPage] = useState(1)
  const { detail } = useTvSeasonDetailContext()
  const fetcher = async () => {
    const result = await RecommendationListTv({ seriesId: detail.id.toString(), params: currentPage.toString() })
    if (result.success) {
      return result.data
    } else {
  ...
  const { data, error, isLoading } = useSWR(`recommendation_tv_${detail.id}_${currentPage}`, fetcher)
  if (error) {
        ...
        <>
          {totalData > 0 ?
            <div className="mt-4 md:px-4">
              ...
              <PaginationTvRecommendation totalPages={data?.total_pages ?? 1} currentPage={currentPage} onPageChange={(newPage) => {
...
export default TvRecommendationsList
